...
:::note
フィーチャーごとに設定した値は、**上書きされるまで保持されます**。エバリュエーターがあるプロパティを返さなくなっても、最後に設定された値がそのまま残ります。マテリアルの値に戻したい場合は、その値を明示的に返してください。マテリアル側のプロパティを変更した場合もすべてのフィーチャーに反映されますが、これはマテリアルの値が実際に変化したときだけです。そのため、エンジンが同じマテリアルを再送しても、フィーチャーごとの設定が失われることはありません。
:::
...
| `facing` | `"upright" \| "flat"` | 矩形を立てるか、地球の接平面に寝かせるか（ポイント／ビルボード／テキスト）。マテリアルの `pointFacing`／`billboardFacing`／`textFacing` を上書きします |
| `rotateWithCamera` | `boolean` | 矩形をカメラに追従して回転させるか（ポイント／ビルボード／テキスト）。`false` でアンカーの東・北・上の座標系に固定されます |
| `rotation` | `number` | アンカーを中心とした平面内の回転角（度、正面から見て時計回り。ポイント／ビルボード／テキスト）。レイヤー内のフィーチャーごとに異なる向きにできます |
...
type EvaluatedValue = {
  ...
  image?: string | null;
  /** 矩形を立てるか、地球の接平面に寝かせるか（ポイント/ビルボード/テキスト） */
  facing?: "upright" | "flat";
  /** 矩形をカメラに追従して回転させるか（ポイント/ビルボード/テキスト） */
  rotateWithCamera?: boolean;
  /** アンカーを中心とした平面内の回転角、度（ポイント/ビルボード/テキスト） */
  rotation?: number;
