...
- Raster DEM rows past the dataset's polar coverage arrive as exact 0 m, and
  the covered row next to them is a blended fraction of the true height. For
  tiles reaching into the band's outer fringe (`polar_nodata_sides`, any zoom),
  `fill_polar_dem_nodata` rewrites the shared DataManager buffer on load (pole
  side only) with the last fully covered row so mesh and hillshade agree; an
  all-zero band-edge tile copies the nearest covered ancestor's region. Fix the
  bytes, never requester status: failed requesters get re-created as Success
